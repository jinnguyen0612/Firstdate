...
class TickProcessRunner extends Command
{
    ...
    /**
     * The function global variable.
     *
     * @var int
     */
    protected $payosExpireTime;

    public function __construct()
    {
        parent::__construct();
        $this->payosExpireTime = (int) env('PAYOS_EXPIRE_TIME', 15);
    }
    ...
    public function handle()
    {
        try {
            ...
            Transaction::where('status', TransactionStatus::Pending->value)
                ->where('created_at', '<=', now()->subMinutes($this->payosExpireTime))
