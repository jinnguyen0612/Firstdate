...
function searchColumsDataTable(datatable, column_search = [], column_date = [], column_select = [], column_select2 = [] ) {
    datatable.api().columns(column_search).every(function () {
        ...
        $(input)
         ...
         // dòng bên dưới được chạy khi tồn tại giá trị default của input (được lấy từ option)
        let selectedVal = $(input).val();
        if (selectedVal != null && selectedVal !== '') {
            column.search(selectedVal, false, false, true).draw();
        }   
    });
}

// disable auto submit of input ( brower's defaut behavior)
...
function createSelectColumnUniqueDatatableAll(input, data){
    ...
    if(Array.isArray(data)){
        data.forEach(item => {
            ...
            if(item.selected){
                option.selected = true; //  Gán selected
                input.value = item.value; //  Đảm bảo input có value sau khi render
            }
            input.append(option);
        });
