        ...
        <div class="row card-body">
            ...
            <div class="col-md-6 col-12">
                <div class="mb-3">
                    ...
                    <x-select style="width: 100%;" name="partner_category_id" class="select2-bs5-ajax" :data-url="route('admin.search.select.partner_category')"
                        id="partner_category_id" :disabled="!$isAdmin">
                        <x-select-option :option="$user->partner_category_id" :value="$user->partner_category_id" :title="$user->partner_category->name" />
                    </x-select>
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="mb-3">
                    ...
                    <x-input name="address" :value="$user->address" placeholder="{{ __('Số nhà, tên đường, phường/xã') }}"
                        :disabled="!$isAdmin" />
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="mb-3">
                    <x-input-pick-address :label="__('Địa chỉ nơi ở hiện tại')" province="province" :valueProvince="$user->province" district="district"
                        :valueDistrict="$user->district" :placeholder="__('Địa chỉ nơi ở hiện tại')" :required="true" :disabled="!$isAdmin" />
                </div>
            </div>
            ...
            <div class="col-12">
                <div class="mb-3">
                    <label class="control-label"><i class="ti ti-clock"></i>
                        {{ __('Thời gian mở cửa') }}:</label>
                    <x-input name="opening_time" :value="old('opening_time', $user->opening_time)" placeholder="{{ __('Thời gian mở cửa') }}" />
                </div>
            </div>

            <div class="col-12">
                <div class="mb-3">
                    <label class="control-label"><i class="ti ti-bookmark"></i> {{ __('Mô tả') }}:</label>
                    <x-textarea maxlength="500" name="description" :required="false"
                        placeholder="{{ __('Nhập mô tả') }}" icon="ti-message" :disabled="!$isAdmin">
                        {{ $user->description }}
                    </x-textarea>
                </div>
            </div>
