...
<div class="transaction-wrapper">
    <div class="transaction-header text-center mb-3">
        ...
        <div class="text-center mb-2">
            <span class="badge {{ TransactionStatus::from($transaction->status)->badge() }} fs-6 py-2 px-3">{{ TransactionStatus::getDescription($transaction->status) }}</span>
        </div>
    ...
    <div class="transaction-body">
        <div class="d-flex justify-content-between mb-3">
            ...
            <span class="fs-5 text-end" style="font-weight: 500;">{{ $transaction->code }} <button class="btn p-1"
                            onclick="copyToClipboard('{{ $transaction->code }}')"><i
                                class="ti ti-copy"></i></button></span>
        </div>
        <div class="d-flex justify-content-between mb-3">
        ...
        <div class="d-flex justify-content-between mb-3">
            ...
            <span class="fs-5 text-end" style="font-weight: 500;">{{ $transaction->receiver ?? 'Hệ thống FirstDate' }}</span>
        </div>
        @else
        <div class="d-flex justify-content-between mb-3">
            <span class="fs-5">Người gửi</span>
            <span class="fs-5 text-end" style="font-weight: 500;">{{ $transaction->sender ?? 'Hệ thống FirstDate' }}</span>
        </div>
        @endif
        <div class="d-flex justify-content-between mb-3">
